...
## Todo:
* Original formulation used 3 yr periods prior to hazard periods to calculate LRS.  There is one period that needs to be interpolated.
* Choose a criteria for using linear/nonlinear prediction
...
tst <- dat[dat$install == worst$install & dat$plot == worst$plot &
           dat$time == worst$time, ]
...
corr
plot(tst$priorbv, tst$priorbv + tst$preds, col = "dark blue", xlab = "prior bv", ylab= "bv",
     main = "Scott LRS with linear prediction")
points(tst$priorbv, tst$bv, col = "orange")
legend("topleft", legend=c("predicted for LRS","observed"),
       pch = 1, col = c("dark blue", "orange"))
...
plot(tst$priorbv, tst$priorbv + tst$preds2, col = "dark blue", xlab = "prior bv", ylab= "bv",
     main = "Scott LRS with power prediction")
points(tst$priorbv, tst$bv, col = "orange")
legend("topleft", legend=c("predicted for LRS","observed"),
       pch = 1, col = c("dark blue", "orange"))
